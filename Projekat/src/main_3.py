...
PATH_TO_CHARTS_FILE = '../results/main_3/'
...
def e(other_condition="", type_of_real_estate="nekretnina", path_and_name_of_file=f'{PATH_TO_CHARTS_FILE}3e).jpg'):
    q = []
    q1 = "SELECT CASE " \
        ...
        "FROM psz_baza.nekretnina " \
        "WHERE tip_ponude = 'Prodaja' " + other_condition + \
        "GROUP BY CASE " \
         "WHEN TRUNCATE(CONVERT(REPLACE(SUBSTRING_INDEX(cena, 'EUR', 1), ' ', ''), UNSIGNED INTEGER), 0) <= 49999 THEN '<= 49999€' " \
        "ELSE '' " \
        "END " \
    ...
    ax1.pie(y, labels=x, autopct='%1.1f%%', shadow=True, startangle=90)
    ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    ax1.title.set_text(f'Broj i procentualni odnos {type_of_real_estate} za prodaju koji pripadaju cenovnom opsegu\n')
    plt.show()
    fig.savefig(f'{path_and_name_of_file}')
