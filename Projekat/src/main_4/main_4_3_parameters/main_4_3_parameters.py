...
PATH_TO_LINEAR_REGRESSION_PARAMETERS = 'w.dat'
...
def fill_minimums_maximums():
    ...
    maximums = []
    q = [
        "select min(convert(left(kvadratura, char_length(kvadratura)-3), signed integer)), max(convert(left(kvadratura, char_length(kvadratura)-3), signed integer)) from nekretnina t where tip_nekretnine = 'Stan' and tip_ponude = 'Prodaja' and substring_index(Lokacija, ',', 1) = 'Beograd' and left(kvadratura, char_length(kvadratura)-3) REGEXP '^[0-9]+$';",
...
def process_one_row_from_database(result):
    ...
    row_data = [square_footage, object_condition, construction_year, floor, num_of_floors, registration, num_of_rooms,
                parking, elevator, location_distance, price]
    try:
    ...
    processed_data = [square_footage, num_of_rooms, price, location_distance]
...
def process_all_data_from_database(results):
    ...
    for result in results:
        try:
            ...
            square_footage, num_of_rooms, price, location_distance = processed_data
        except TypeError:
            continue
        xx.append([square_footage, num_of_rooms, location_distance])
        yy.append(price)
        zz.append(row_data)
    return xx, yy, zz
...
def test_data():
    ...
    table = prettytable.PrettyTable(
        ['Predvidena vrednost', 'Prava vrednost', 'Greška', 'Povrsina stana', 'Stanje nekretnine', 'Godina izgradnje',
         'Sprat', 'Spratnost', 'Uknjizenost', 'Broj soba', 'Parking', 'Lift', 'Udaljenost lokacije'])
...
def gradient_descent():
    global w
    # w = [1036947.8508718222, 1279644.9309793701, -72616.9815483902, -248239.5200840906]
...
def get_location_distance_for_one_data(location):
    ...
    try:
        ...
        time.sleep(10)
        distance = driver.find_elements_by_xpath("//div[@class='xB1mrd-T3iPGc-iSfDt-ij8cu']//div//div//div//div")[
            0].text
...
def normalize_training_set():
    for i in range(0, m):
        print('***', len(x[i]))
        for j in range(0, n):
            x[i][j] = ((x[i][j] - minimums_of_variables[j]) / (
                        maximums_of_variables[j] - minimums_of_variables[j])) * 2 - 1
    for i in range(0, n):
    ...
    for i in range(0, len(test_data_x)):
        for j in range(0, n):
            if maximums_of_variables[j] == minimums_of_variables[j]:
                ...
                test_data_x[i][j] = ((test_data_x[i][j] - minimums_of_variables[j]) / (
                            maximums_of_variables[j] - minimums_of_variables[j])) * 2 - 1
...
def input_form():
    ...
    input_data = [square_footage, num_of_rooms, location_distance]
    for i in range(0, n):
        input_data[i] = ((input_data[i] - minimums_of_variables[i]) / (
                    maximums_of_variables[i] - minimums_of_variables[i])) * 2 - 1
    print("Višestruka linarna regersija korišćenjem gradijentnog spusta predviđa da je cena stana: ",
          round(calculate(input_data, 0)[2], 2), "EUR-a.")
