...
def process_one_row_from_database(result):
    ...
    processed_data = [square_footage, location_distance, price]
...
def process_all_data_from_database(results):
    ...
    for result in results:
        try:
            ...
            square_footage, location_distance, price = processed_data
        except TypeError:
            continue
        ...
        xx.append([square_footage, location_distance])
        yy.append(price)
        zz.append(row_data)
    return xx, yy, zz
...
def gradient_descent():
    global w
    # [1007669.2154251635, 1187563.6561936412, -249234.6903088075]
...
def input_form():
    ...
    input_data = [square_footage, location_distance]
    for i in range(0, n):
...
def linear_regression_2_parameters(square_footage, location_distance):
    input_data = [square_footage, location_distance]
    for i in range(0, n):
        input_data[i] = ((input_data[i] - minimums_of_variables[i]) / (
                    maximums_of_variables[i] - minimums_of_variables[i])) * 2 - 1
    print("Višestruka linarna regersija korišćenjem gradijentnog spusta i 2 odlike (kvadratura, lokacija) predviđa da je cena stana: ", round(calculate(input_data, 0)[0], 2), "EUR-a.")
...
# 1389579 . w after [1007669.2154251635, 1187563.6561936412, -249234.6903088075] -5.245208740234375e-05
# 1389580 . w after [1007669.2156545337, 1187563.6564188383, -249234.69026875123] 5.7220458984375e-06