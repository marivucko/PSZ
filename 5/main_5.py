...
import math
import prettytable
# from main_4 import get_location_distance_for_one_data
import mysql.connector as mysql
...
minimums_of_variables = []
maximums_of_variables = []
...
def update_min_max(result):
    global minimums_of_variables, maximums_of_variables
    global n
    for i in range(0, len(result)):
        if result[i] < minimums_of_variables[i]:
            minimums_of_variables[i] = result[i]
        if result[i] > maximums_of_variables[i]:
            maximums_of_variables[i] = result[i]
...
def process_all_data_from_database(results):
    ...
    k = 0
    for i in range(0, 10):
        maximums_of_variables.append(float(0))
    for result in results:
        try:
            ...
            continue
        if k == 0:
            minimums_of_variables.append(square_footage)
            minimums_of_variables.append(object_condition)
            minimums_of_variables.append(construction_year)
            minimums_of_variables.append(floor)
            minimums_of_variables.append(num_of_floors)
            minimums_of_variables.append(float(-1))  # minimum for registration (-1 not registerd, 1 registered)
            minimums_of_variables.append(num_of_rooms)
            minimums_of_variables.append(float(-1))  # minimum for parking (no parking)
            minimums_of_variables.append(float(-1))  # minimum for elevator (no elevator)
            minimums_of_variables.append(location_distance)
            k = k + 1
        update_min_max([square_footage, object_condition, construction_year, floor, num_of_floors, registration, num_of_rooms, parking, elevator, location_distance])
...
def normalize_input_data(z):
    for j in range(0, n):
        if maximums_of_variables[j] == minimums_of_variables[j]:
            z[j] = 0
        else:
            z[j] = ((z[j] - minimums_of_variables[j]) / (maximums_of_variables[j] - minimums_of_variables[j])) * 2 - 1
    return z


def get_class_from_distance(z, w, euclidean):
    # z = normalize_input_data(z)
    var_x = []
    for i in range(0, m):
        distance = 0
        for j in range(0, n):
